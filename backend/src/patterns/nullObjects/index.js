/**
 * Null Objects Index
 * Central export point for all null object implementations
 */

export { NullCar, NULL_CAR } from './NullCar.js';
export { NullIndustry, NULL_INDUSTRY } from './NullIndustry.js';
export { NullTrain, NULL_TRAIN } from './NullTrain.js';
export { NullRoute, NULL_ROUTE } from './NullRoute.js';
export { NullCarOrder, NULL_CAR_ORDER } from './NullCarOrder.js';
export { NullOperatingSession, NULL_OPERATING_SESSION } from './NullOperatingSession.js';
export { NullLocomotive, NULL_LOCOMOTIVE } from './NullLocomotive.js';
export { NullStation, NULL_STATION } from './NullStation.js';
export { NullAarType, NULL_AAR_TYPE } from './NullAarType.js';
export { NullBlock, NULL_BLOCK } from './NullBlock.js';
export { NullTrack, NULL_TRACK } from './NullTrack.js';
export { NullGood, NULL_GOOD } from './NullGood.js';
